...
export default function ActiveBets() {
        ...
        const fetchActiveBets = async () => {
            try {
                ...
                if (response.data.status === 'success' && Array.isArray(response.data.data)) {
                    ...
                    const displayedBets = transformedBets.slice(0, 2)
                    setActiveBets(displayedBets)
                } else {
    ...
    return (
        <div className=" space-y-6 p-4 bg-gradient-to-b from-gray-900 to-gray-800">
            ...
            >
                <h1 className="text-3xl font-bold mb-4 bg-gradient-to-r from-emerald-500 to-emerald-700 bg-clip-text text-transparent">
                    Active Bets
                </h1>
                <p className="text-gray-400 text-1xl">Place your bets on upcoming placements</p>
            </motion.div>

            <AnimatePresence>
                <motion.div
                    className="flex flex-col md:flex gap-6 px-2"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
